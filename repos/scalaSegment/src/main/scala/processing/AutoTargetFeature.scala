...
//          Some(newState -> MKTask(s"$taskName - $subTaskName", MKActions(mouseActions, keyboardActions)))
//
//        case "target chosen" =>
...
//
//    // 3) Merge and return
//    (afterBattleState, battleMouse ++ screenMouse, List.empty)
//  }
//
//
...
//          else "none"
//
////        def dirToKeys(dirOpt: Option[String]): List[KeyboardAction] =
////          dirOpt.toList.flatMap {
////            case d if d.startsWith("Move") =>
////              val key = d.stripPrefix("Move")
////              key match {
////                case "Up"    => List(PressKey.fromKeyString("Up"))
////                case "Down"  => List(PressKey.fromKeyString("Down"))
////                case "Left"  => List(PressKey.fromKeyString("Left"))
////                case "Right" => List(PressKey.fromKeyString("Right"))
////                case "UpLeft"   => List("Up","Left").map(PressKey.fromKeyString)
////                case "UpRight"  => List("Up","Right").map(PressKey.fromKeyString)
////                case "DownLeft" => List("Down","Left").map(PressKey.fromKeyString)
////                case "DownRight"=> List("Down","Right").map(PressKey.fromKeyString)
////                case _ => Nil
////              }
//            case _ => Nil
//          }
...
//            val gs1 = generateSubwaypointsToCreature(target, baseState, json)
//            if (gs1.autoTarget.subWaypoints.nonEmpty) {
//              val next   = gs1.autoTarget.subWaypoints.head
//              val dirOpt = calculateDirection(presentLoc, next, gs1.characterInfo.lastDirection)
//              val keys   = dirToKeys(dirOpt)
//              val gs2 = gs1.copy(
//                characterInfo = gs1.characterInfo.copy(lastDirection = dirOpt),
//                autoTarget    = gs1.autoTarget.copy(subWaypoints = gs1.autoTarget.subWaypoints.tail)
//              )
//              (gs2, Nil, keys)
//            } else (gs1, Nil, Nil)
//
...
//            )
//            if (dist < 3) {
//              val gs1 = generateSubwaypointsToEscape(creatureLocation = Vec(creatureData.posX, creatureData.posY), baseState, json)
//              if (gs1.autoTarget.subWaypoints.nonEmpty) {
//                val next   = gs1.autoTarget.subWaypoints.head
//                val dirOpt = calculateDirection(presentLoc, next, gs1.characterInfo.lastDirection)
//                val keys   = dirToKeys(dirOpt)
...
//
//            val gs1 = generateSubwaypointsToCreature(targetPos, baseState, json)
//            if (gs1.autoTarget.subWaypoints.nonEmpty) {
//              val next   = gs1.autoTarget.subWaypoints.head
//              val dirOpt = calculateDirection(presentLoc, next, gs1.characterInfo.lastDirection)
//              val keys   = dirToKeys(dirOpt)
...
//    println(s"[DEBUG] Character location: $presentLoc, target: $targetLocation")
//
//    // 3) compute path if not already at target
//    val rawPath: List[Vec] =
...
//        Nil
//      }
//    println(s"[DEBUG] Path to creature: $rawPath")
//
//    // 4) drop the starting point
//    val filteredPath = rawPath.filterNot(_ == presentLoc)
//    println(s"[DEBUG] Filtered path: $filteredPath")
//
//    // 5) update caveBot sub‐state immutably
...
//
//    val newState = state.copy(autoTarget = newAT)
//    (newState, clickActions, List.empty)
//  }
//
//
...
//    (state.copy(autoTarget = at3), List.empty, List.empty)
//  }
//
//
