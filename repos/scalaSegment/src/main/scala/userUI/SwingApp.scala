...
class SwingApp(playerClassList: List[Player],
  ...
  try {
    ...
    // Initialize your bots and UI components here...
    initializeComponents()

    checkAutorunOnStartup()
    ...
    case ex: Exception =>
      println(s"Error in SwingApp constructor: ${ex.getMessage}")
      ex.printStackTrace()
      System.exit(-1)
  }
  ...
  private var autoHealCheckbox: CheckBox = _
  private var hotkeysCheckbox: CheckBox = _
  private var runeMakerCheckbox: CheckBox = _
  private var trainingCheckbox: CheckBox = _
  private var hotkeyModeCheckbox: CheckBox = _
  private var debugModeCheckbox: CheckBox = _
  private var shareDataModeCheckbox: CheckBox = _
  private var caveBotCheckbox: CheckBox = _
  private var teamHuntCheckbox: CheckBox = _
  private var fishingCheckbox: CheckBox = _
  private var autoResponderCheckbox: CheckBox = _
  private var mouseMovementsCheckbox: CheckBox = _
  private var guardianCheckbox: CheckBox = _
  private var autoTargetCheckbox: CheckBox = _
  private var autoLootCheckbox: CheckBox = _

  // Declare bot instances as class-level variables
  private var autoHealBot: AutoHealBot = _
  private var hotkeysBot: HotkeysBot = _
  private var caveBotBot: CaveBotBot = _
  private var autoTargetBot: AutoTargetBot = _
  private var autoLootBot: AutoLootBot = _
  private var runeMakerBot: RuneMakerBot = _
  private var fishingBot: FishingBot = _
  private var guardianBot: GuardianBot = _
  private var trainingBot: TrainingBot = _
  private var autoResponderBot: AutoResponderBot = _
  private var teamHuntBot: TeamHuntBot = _
  ...
  def applyHealingSettings(healingSettings: HealingSettings): Unit = {
    ...
    autoHealBot.lifeRingManaMinField.text = healingSettings.lifeRingManaMin.toString
    autoHealBot.lifeRingManaMaxField.text = healingSettings.lifeRingManaMax.toString
    autoHealBot.healingRingManaMinField.text = healingSettings.healingRingManaMin.toString
    autoHealBot.healingRingManaMaxField.text = healingSettings.healingRingManaMax.toString

    autoHealBot.energyRingHealthMinField.text = healingSettings.energyRingHealthMin.toString
    autoHealBot.energyRingHealthMaxField.text = healingSettings.energyRingHealthMax.toString
    autoHealBot.energyRingManaMinField.text = healingSettings.energyRingManaMin.toString
    autoHealBot.energyRingManaMaxField.text = healingSettings.energyRingManaMax.toString
  ...
  private def checkAutorunOnStartup(): Unit = {
    AutorunManager.checkAndLoadAutorunSettings().flatMap {
      case Some(settings) =>
        AutorunManager.handleAutorun(
          settings,
          applySettingsToUI,
          () => executeRunLogic(settings)
        )
      case None =>
        IO.unit // No autorun file found, continue normally
    }.unsafeRunAndForget()
  }

  private def executeRunLogic(settings: UISettings): Unit = {
    // Update the shared Ref
    settingsRef.set(settings).unsafeRunAndForget()

    // Send messages to actors (same as RUN button logic)
    println(s"[UI] starting with settings: $settings")
    uiAppActor ! MainApp.StartActors(settings)
    periodicFunctionActorRef ! MainApp.StartActors(settings)
  }
  ...
  private def initializeComponents(): Unit = {
    ...
    autoHealCheckbox = new CheckBox("Auto Heal")
    hotkeysCheckbox = new CheckBox("Hotkeys")
    runeMakerCheckbox = new CheckBox("Rune Maker")
    trainingCheckbox = new CheckBox("Training")
    caveBotCheckbox = new CheckBox("Cave Bot")
    teamHuntCheckbox = new CheckBox("Team Hunt")
    fishingCheckbox = new CheckBox("Fishing")
    autoResponderCheckbox = new CheckBox("Auto Responder")
    mouseMovementsCheckbox = new CheckBox("Mouse Movements")
    guardianCheckbox = new CheckBox("Guardian")
    autoTargetCheckbox = new CheckBox("Auto Target")
    autoLootCheckbox = new CheckBox("Auto Loot")
    hotkeyModeCheckbox = new CheckBox("Hotkey Mode")
    debugModeCheckbox = new CheckBox("Debug Mode")
    shareDataModeCheckbox = new CheckBox("Share Data")

    // Initialize AutoHeal class
    //  val mainBot = new MainBot(currentPlayer, uiAppActor, jsonProcessorActor)
    autoHealBot = new AutoHealBot(uiAppActor, jsonProcessorActor, settingsRef)
    hotkeysBot = new HotkeysBot(currentPlayer, uiAppActor, jsonProcessorActor)
    caveBotBot = new CaveBotBot(currentPlayer, uiAppActor, jsonProcessorActor)
    autoTargetBot = new AutoTargetBot(currentPlayer, uiAppActor, jsonProcessorActor)
    autoLootBot = new AutoLootBot(currentPlayer, uiAppActor, jsonProcessorActor)
    runeMakerBot = new RuneMakerBot(currentPlayer, uiAppActor, jsonProcessorActor)
    fishingBot = new FishingBot(uiAppActor, jsonProcessorActor, settingsRef)

    guardianBot = new GuardianBot(currentPlayer, uiAppActor, jsonProcessorActor)
    trainingBot = new TrainingBot(currentPlayer, uiAppActor, jsonProcessorActor)
    autoResponderBot = new AutoResponderBot(currentPlayer, uiAppActor, jsonProcessorActor)
    teamHuntBot = new TeamHuntBot(currentPlayer, uiAppActor, jsonProcessorActor)
    ...
    def collectHealingSettings(): HealingSettings = {
      ...
      HealingSettings(
        ...
        lifeRingManaMin = parseTextFieldToInt(autoHealBot.lifeRingManaMinField.text),
        lifeRingManaMax = parseTextFieldToInt(autoHealBot.lifeRingManaMaxField.text),
        healingRingManaMin = parseTextFieldToInt(autoHealBot.healingRingManaMinField.text),
        healingRingManaMax = parseTextFieldToInt(autoHealBot.healingRingManaMaxField.text),

        energyRingHealthMin = parseTextFieldToInt(autoHealBot.energyRingHealthMinField.text),
        energyRingHealthMax = parseTextFieldToInt(autoHealBot.energyRingHealthMaxField.text),
        energyRingManaMin = parseTextFieldToInt(autoHealBot.energyRingManaMinField.text),
        energyRingManaMax = parseTextFieldToInt(autoHealBot.energyRingManaMaxField.text),
    ...
    def collectSettingsFromUI(): UISettings = UISettings(
      ...
      hotkeyMode = hotkeyModeCheckbox.selected,
      debugMode = debugModeCheckbox.selected,
      shareDataMode = shareDataModeCheckbox.selected,
    )
    ...
    listenTo(autoHealCheckbox, runeMakerCheckbox, trainingCheckbox, caveBotCheckbox,
      ...
      autoLootCheckbox,teamHuntCheckbox,hotkeysCheckbox, debugModeCheckbox,
      shareDataModeCheckbox, hotkeyModeCheckbox,
    )

    reactions += {
      case ButtonClicked(component) if component == autoHealCheckbox =>
        println("Auto Heal Checkbox clicked")

      case ButtonClicked(component) if component == hotkeysCheckbox =>
        println("Hotkeys Checkbox clicked")

      case ButtonClicked(component) if component == runeMakerCheckbox =>
        println("Rune Maker Checkbox clicked")

      case ButtonClicked(component) if component == trainingCheckbox =>
        println("Training Checkbox clicked")

      case ButtonClicked(component) if component == caveBotCheckbox =>
        println("Cave Bot Checkbox clicked")

      case ButtonClicked(component) if component == teamHuntCheckbox =>
        println("Team Hunt Checkbox clicked")

      case ButtonClicked(component) if component == autoTargetCheckbox =>
        println("Auto Target Checkbox clicked")

      case ButtonClicked(component) if component == guardianCheckbox =>
        println("Guardian Checkbox clicked")

      case ButtonClicked(component) if component == autoResponderCheckbox =>
        println("Auto Responder Checkbox clicked")

      case ButtonClicked(component) if component == fishingCheckbox =>
        println("Fishing Checkbox clicked")

      case ButtonClicked(component) if component == mouseMovementsCheckbox =>
        println("Mouse Movements Checkbox clicked")

      case ButtonClicked(component) if component == autoLootCheckbox =>
        println("Auto Loot Checkbox clicked")

      case ButtonClicked(component) if component == debugModeCheckbox =>
        println("Debug Mode Checkbox clicked")

      case ButtonClicked(component) if component == shareDataModeCheckbox =>
        println("Share Data Mode Checkbox clicked")

      case ButtonClicked(component) if component == shareDataModeCheckbox =>
        println("Hotkey Mode Checkbox clicked")
    }
    ...
    contents = new TabbedPane {
      pages += new TabbedPane.Page("Main", new Component {
        ...
        val checkBoxComponents = Seq(autoHealCheckbox, hotkeysCheckbox, runeMakerCheckbox, trainingCheckbox,
          ...
          guardianCheckbox, autoTargetCheckbox, autoLootCheckbox, debugModeCheckbox, shareDataModeCheckbox,
          hotkeyModeCheckbox)
