...
bool TcpServer::PollRequest(SOCKET& sckFd, sockaddr_in& clientAddr, std::string& requestStr)
{
    ...
    {
        std::cerr << "[TcpServer] Exception in PollRequest: " << e.what() << std::endl;
        // Optionally handle the exception or re-throw to let the caller handle it
        // throw; // Uncomment if re-throwing
    }
    return exitQuery;
}
